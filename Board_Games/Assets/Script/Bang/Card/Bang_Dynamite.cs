...
public class Bang_Dynamite : Bang_Card
{
    ...
    {
        My_Type = Bang_Game_Manager.BANG_CARD.DYNAMITE;
        ...
        Front_Sprite = Bang_Image_Manager.Instance.Get_Card_Sprite(Bang_Game_Manager.BANG_CARD.DYNAMITE);
    ...
    public override void Card_Use(int Player_Num, int Target_Num = -1)
    {
        ...
        Bang_Message_Manager.Instance.Create_Message(Bang_Game_Manager.Instance.Player_List[Player_Num].GetComponent<Bang_Player>().name + "이(가) 다이너마이트 카드를 사용합니다.", 1);
        
        Bang_Game_Manager.Instance.Player_List[Player_Num].GetComponent<Bang_Player>().Item_Dynamite_Active(gameObject);
