...
public class Bang_Barrel : Bang_Card
{
    ...
    {
        My_Type = Bang_Game_Manager.BANG_CARD.BARREL;
        ...
        Front_Sprite = Bang_Image_Manager.Instance.Get_Card_Sprite(Bang_Game_Manager.BANG_CARD.BARREL);
    ...
    public override void Card_Use(int Player_Num, int Target_Num = -1)
    {
        ...
        Bang_Message_Manager.Instance.Create_Message(Bang_Game_Manager.Instance.Player_List[Player_Num].GetComponent<Bang_Player>().name + "이(가) 술통 카드를 사용합니다.", 1);

        Bang_Game_Manager.Instance.Player_List[Player_Num].GetComponent<Bang_Player>().Item_Barrel_Active(gameObject);
    ...
    {
        return Bang_Game_Manager.Instance.Get_Bot_Barrel_Use_Check();
    }
}
