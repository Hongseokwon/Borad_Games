...
public class Bang_Scope : Bang_Card
{
    ...
    {
        My_Type = Bang_Game_Manager.BANG_CARD.SCOPE;
        ...
        Front_Sprite = Bang_Image_Manager.Instance.Get_Card_Sprite(Bang_Game_Manager.BANG_CARD.SCOPE);
    ...
    public override void Card_Use(int Player_Num, int Target_Num = -1)
    {
        ...
        Bang_Message_Manager.Instance.Create_Message(Bang_Game_Manager.Instance.Player_List[Player_Num].GetComponent<Bang_Player>().name + "이(가) 조준경 카드를 사용합니다.", 1);

        Bang_Game_Manager.Instance.Player_List[Player_Num].GetComponent<Bang_Player>().Item_Scope_Active(gameObject);
    ...
    {
        return Bang_Game_Manager.Instance.Get_Bot_Scope_Use_Check();
    }
}
