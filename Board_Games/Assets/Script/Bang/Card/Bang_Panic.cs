...
public class Bang_Panic : Bang_Card
{
    ...
    {
        My_Type = Bang_Game_Manager.BANG_CARD.PANIC;
        ...
        Front_Sprite = Bang_Image_Manager.Instance.Get_Card_Sprite(Bang_Game_Manager.BANG_CARD.PANIC);
    }
    ...
    public override void Card_Use(int Player_Num, int Target_Num = 0)
    {
        ...
        Bang_Message_Manager.Instance.Create_Message(Str_Player + "이(가) 강탈 카드를 사용합니다.", 1);

        Bang_Game_Manager.Instance.Panic_Use(Player_Num, Target_Num);
    ...
    public override void Card_Use_Pre()
    {
        ...
        Bang_UI_Manager.Instance.User_Panic_Target_Select();
    }

    public override bool Bot_Card_Use_Check()
    {
        return Bang_Game_Manager.Instance.Player_List[Bang_Game_Manager.Instance.Get_Now_Player()].GetComponent<Bang_Bot>().Panic_Check();
    }

    public override void Bot_Card_Use_Pre()
    {
        int n = Bang_Game_Manager.Instance.Get_Now_Player();
        Card_Use(n, Bang_Game_Manager.Instance.Player_List[n].GetComponent<Bang_Bot>().Panic_Target_Find());
    }
}
